...
impl Rectangle {
	...
	pub fn collides_with_player(&self, other: &Rectangle) -> bool {
		self.right() >= other.left() + units::Game(15.0) &&
		self.left() <= other.right() - units::Game(15.0) &&
		self.top() <= other.bottom() - units::Game(15.0) &&
		self.bottom() >= other.top() + units::Game(15.0)
	}
}
